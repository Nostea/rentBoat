...
const ReservationCard = ({ reservationId, startDate, endDate, name, reservations, setReservations }) => {
  ...
  const deleteReservation = () => {
    fetch(`${backendUrl}/api/v1/reservations/${reservationId}`, {
      ...
      .then((deletedReservation) => {
        setReservations(reservations.filter((singleRes) => singleRes._id !== deletedReservation._id));
      })
  ...
  return (
    <article className=" bg-white flex flex-col p-6">
      ...
      <div className="py-4">
        {" "}
        <button type="button" className="bg-pink px-4 py-2 rounded-lg hover:bg-skyblue" onClick={deleteReservation}>
          Delete
        </button>
        <button type="button" className="bg-lightpink px-4 py-2 rounded-lg hover:bg-skyblue" onClick={editReservation}>
          Edit
        </button>
      </div>
    </article>
